...
async def extract_url_content(
    ...
    """Tiered URL → content pipeline.

    Delegates to ``app.services.imports.ImportOrchestrator``. The return shape
    matches the legacy contract (title / excerpt / body / has_img / img_url /
    partial / failure_reason) and additionally includes a ``diagnostics`` dict
    with the strategy used, challenge classification, per-stage timings, and
    fallback count. Callers that ignored the old shape continue to work.
    """
    from app.services.imports import ImportOrchestrator, ImportRequest

    req = ImportRequest(
        url=url,
        camoufox_enabled=camoufox_enabled,
        camoufox_url=camoufox_url,
        camoufox_timeout=camoufox_timeout,
        flaresolverr_url=flaresolverr_url,
        browserless_enabled=browserless_enabled,
        browserless_url=browserless_url,
        browserless_token=browserless_token,
        browserless_timeout=browserless_timeout,
        static_fetch_timeout=static_fetch_timeout,
    )
